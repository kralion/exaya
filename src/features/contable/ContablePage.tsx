...
export function ContablePage() {
  ...
  const onChangeRuta = (rutaId: string) => {
    if (!salidasDiarias?.response) return;
  ...
  const totalTravelEncomiendasIncome =
    currentViaje?.response?.encomiendas.reduce(
      (total: number, encomienda: { precio: number }) =>
        total + encomienda.precio,
      0
    ) ?? 0;
  const totalTravelBoletosIncome =
    currentViaje?.response?.boletos.reduce(
      (total: number, boleto: { precio: number }) => total + boleto.precio,
      0
    ) ?? 0;
          ...
          <Space className="flex flex-col items-start justify-between lg:flex-row">
                  ...
                  horarios.map((horario, index) => {
                    ...
                    return (
                      <Button
                        ...
                        onClick={() => setCurrentViajeId(viajeId as string)}
                      >
            ...
            <Space className="gap-4">
              ...
              <Space direction="vertical">
                ...
                <Select
                  placeholder="Ruta"
                  onChange={(rutaId: string) => onChangeRuta(rutaId)}
                  loading={isLoading}
                  style={{ width: 210 }}
                  disabled={
                ...
                >
                  {salidasDiarias?.response &&
                    getUniqueRoutes(salidasDiarias).map(([rutaString, rutaId]) => (
                      <Select.Option key={rutaString} value={rutaId}>
                        {rutaString}
                      </Select.Option>
                    ))}
                </Select>
