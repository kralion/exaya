...
const TravelTicketPrint = forwardRef<
  HTMLDivElement,
  { id: string; showPrintComponent: boolean }
>(function TravelTicketPrint({ id, showPrintComponent }, ref) {
  const { data } = api.boletos.getBoletosById.useQuery({ id });
  if (data?.response === null) {
    return (
      <div
        style={{
          display: showPrintComponent ? "block" : "none",
        }}
        ref={ref}
      ...
      <footer className="flex justify-between rounded-t-lg bg-yellow-700 p-4  text-black">
        ...
        <div>
          ...
          <p className="text-sm">
            ...
            {data?.response?.viaje.ruta.ciudadOrigen.slice(0, 3).toUpperCase()}
          </p>
          <p className="text-sm">
            Llegada: {data?.response?.viaje.ruta.ciudadDestino},{" "}
            {data?.response?.viaje.ruta.ciudadDestino.slice(0, 3).toUpperCase()}
          </p>
