...
import type { EncomiendasContextProps, Encomienda } from '~/interfaces/interfaces';
import { encomiendas as INITIAL_DATA } from "~/data/encomiendas.json";
export const EncomiendasContext = createContext<EncomiendasContextProps>({
    encomiendas: INITIAL_DATA,
...
export const EncomiendasContextProvider = ({ children }: Props) => {
    const [encomiendas, setEncomiendas] = useState<Encomienda[]>(INITIAL_DATA as Encomienda[])
    ...
    const handleDeleteEncomienda = (key: string) => {
        const newEncomiendas = encomiendas.filter(encomienda => encomienda.key !== key)
    ...
    return (
        ...
        <EncomiendasContext.Provider value={{
            encomiendas,
            handleAddEncomienda,
...
export function useEncomiendasContext() {
    ...
    if (!context) {
        throw new Error('useEncomiendasContext debe estar dentro del proveedor EncomiendasContext');
    }
    return context;
}
